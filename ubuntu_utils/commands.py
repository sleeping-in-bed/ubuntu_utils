...
from ubuntu_utils.framework.remote import Remote
from ubuntu_utils.framework.settings import Settings
from ubuntu_utils.framework.utils import ConfigEditor
...
def apt_offline_install():
    ...
    r.exec(f"sudo apt install -y {Settings.resources_dir / 'apt-offline_1.8.6-1_all.deb'}")
    file = None
...
def install_python_dev_env():
    # install python
    r.exec("sudo apt-get install -y build-essential python3-dev python3-pip python-is-python3")
    r.exec("sudo apt-get install -y python3-tk")  # to use the pyautogui
    r.exec("sudo apt install -y python3-docker")   # install other packages
    # install pyenv
    r.exec("curl -fsSL https://pyenv.run | bash",
           ignore_error=[
               re.compile(r"WARNING: Can not proceed with installation. Kindly remove the '.*' directory first.")], )
...
""")
    # install uv
    # add PATH before uv
    r.exec(r"""grep -Fxq 'export PATH="$HOME/.local/bin:$PATH"' ~/.bashrc || echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc""")
    # install uv
...
def pre_settings():
    ...
    if configs.HAS_GUI:
        ...
        r.replace("/etc/systemd/logind.conf",
                  "#HandleLidSwitch=suspend",
                  "HandleLidSwitch=ignore",
                  )
        # auto login without input password
        ce = ConfigEditor("/etc/gdm3/custom.conf")
        ce.append("AutomaticLoginEnable", "true")
        ce.append("AutomaticLogin", r.user)
        # enable root login
        ...
        # install some gui software, baobab is the disk usage analyzer
        r.exec("sudo apt-get -y install baobab gedit vlc")
...
def ip_configuration():
    ...
    ens_name = r.capture("ip link show | grep -oE 'ens[0-9]+'").stdout  # search Ethernet device name
...
def install_anaconda(installation_dir=Path("~/anaconda3")):
    save_path = remote.get_file("software/Anaconda3-2024.10-1-Linux-x86_64.sh")
    if not save_path:
        save_path = "/tmp/Anaconda3-2024.10-1-Linux-x86_64.sh"
        r.exec(f"curl -o {save_path} https://repo.anaconda.com/archive/Anaconda3-2024.10-1-Linux-x86_64.sh")
...
sudo tee /etc/apt/sources.list.d/docker.list > /dev/null')
    ...
    if version:
        ...
        r.exec("sudo apt-get install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin -y")
...
def restore_docker_images():
    r.chdir(Settings.scripts_dir)
    for remote_path in remote.get_file_paths("docker/images"):
        file = remote.get_file(f"docker/images/{remote_path}")
        r.exec(f'sudo ./xz-docker -d "{file}"', no_check=True)
...
gpg --batch --pinentry-mode loopback --passphrase "" --generate-key keyparams""")
    output = r.capture(f"gpg --list-secret-keys {configs.EMAIL}", no_check=True).stdout
    match = re.search(rf"sec.*?\n(.*?)\nuid.*?{configs.NAME} <{configs.EMAIL}>", output)
    key = match.group(1).strip() if match else ""
    # To initialize pass, run the following command using the public key generated from the previous command
    r.exec("sudo apt update && sudo apt install -y pass")
    r.exec(f"pass init {key}")

    # Login
    r.exec(f'echo "{configs.DOCKERHUB_PASSWORD}" | docker login -u {configs.DOCKERHUB_NAME} --password-stdin')
...
def add_registry_certificate_to_trusted():
    ...
    r.exec(f'sudo sshpass -p "{configs.PASSWORD}" scp -o StrictHostKeyChecking=no '
...
def install_vscode():
    if configs.HAS_GUI:
        r.exec("""sudo apt install -y wget gpg &&\
...
def install_cursor():
    if configs.HAS_GUI:
        save_path = remote.get_file("software/Cursor-0.50.5-x86_64.AppImage")
        if not save_path:
            save_path = "/tmp/Cursor-0.50.5-x86_64.AppImage"
            r.exec(f"curl -o {save_path} https://downloads.cursor.com/production/96e5b01ca25f8fbd4c4c10bc69b15f6228c80771/linux/x64/Cursor-0.50.5-x86_64.AppImage")
        r.exec(f"cp {save_path} {r.home / 'Cursor-0.50.5-x86_64.AppImage'}")
        r.exec(f"chmod +x {r.home / 'Cursor-0.50.5-x86_64.AppImage'}")
        r.exec("sudo apt install -y libfuse2")
        r.exec("""sudo bash -c 'echo -e "#!/bin/bash\\n~/Cursor-0.50.5-x86_64.AppImage --no-sandbox" > /usr/local/bin/cursor && chmod +x /usr/local/bin/cursor'""")
        # remove installation package
        r.exec(f"sudo rm -f {save_path}")


def install_remote_ide():
    r.chdir(Settings.scripts_dir)
    r.exec(f"sudo ./get_docker_file wantodie/build-data:2024.1.7 /build-data/data.tar.xz /tmp")
    r.exec(f"tar -xf /tmp/data.tar.xz -C ~ --strip-components=1")
    pycharm_dir = "~/.cache/JetBrains/RemoteDev/dist/52c7d266bab24_pycharm-professional-2024.1.7/bin"
    webstorm_dir = "~/.cache/JetBrains/RemoteDev/dist/40963d2865026_WebStorm-2024.1.7/bin"
    r.exec(f"""sudo ln -s {pycharm_dir}/pycharm.sh /usr/local/bin/pycharm &&
sudo ln -s {webstorm_dir}/webstorm.sh /usr/local/bin/webstorm""")
    if configs.HAS_GUI:
        r.exec(f"./shortcut {pycharm_dir}/pycharm.sh -n pycharm -i {pycharm_dir}/pycharm.png -t false")
        r.exec(f"./shortcut {webstorm_dir}/webstorm.sh -n webstorm -i {webstorm_dir}/webstorm.png -t false")


# def install_pycharm(remote_version: bool = True, version: str = "2023.3.3"):
#     if remote_version:
#         save_path = remote.get_file("docker/files/pycharm.tar.xz")
...
def install_chrome():
    if configs.HAS_GUI:
        ...
        if not save_path:
            save_path = "/tmp/google-chrome-stable_current_amd64.deb"
            r.exec(f"curl -o {save_path} https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb")
...
def install_sougoupinyin():
    if configs.HAS_GUI:
        ...
        r.exec("sudo apt install -y libqt5qml5 libqt5quick5 libqt5quickwidgets5 qml-module-qtquick2 libgsettings-qt1")
        r.exec(f"sudo dpkg -i {save_path}")
        r.exec("gsettings set org.gnome.desktop.input-sources sources \"[('xkb', 'fcitx')]\"")
...
sudo modprobe -- nf_conntrack""")
    ...
    r.replace("/etc/containerd/config.toml", "SystemdCgroup = false", "SystemdCgroup = true")
    r.replace("/etc/containerd/config.toml",
    ...
    r.exec("curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.32/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg")
    r.exec("echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.32/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list")
    r.exec("sudo apt-get update", uid=1)
    r.exec(f"sudo apt-get install -y kubelet={version} kubeadm={version} kubectl={version}")
    r.exec("sudo apt-mark hold kubelet kubeadm kubectl")

    r.replace("/etc/default/kubelet",
              "KUBELET_EXTRA_ARGS=",
...
sudo dpkg -i minikube_latest_amd64.deb""")
    r.exec("sudo minikube start --force")
...
sudo chown $(id -u):$(id -g) $HOME/.kube/config""")
    # install calico
    r.exec("kubectl create -f https://raw.githubusercontent.com/projectcalico/calico/v3.29.2/manifests/tigera-operator.yaml")
